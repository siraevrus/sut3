...
// Проверяем, что пользователь может создавать запросы
$user = getCurrentUser();
            ...
            try {
                ...
                if (!$template) {
                    ...
                    if (!$warehouse) {
                        ...
                        // Если все валидно, сохраняем запрос
                        if (empty($errors)) {
                            $stmt = $pdo->prepare("
                            ...
                            $_SESSION['success_message'] = 'Запрос успешно отправлен';
                ...
                logError('Request creation error: ' . $e->getMessage());
...
try {
    ...
    logError('Request create page error: ' . $e->getMessage());
    $templates = [];
    $warehouses = [];
}
