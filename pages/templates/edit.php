...
try {
    ...
    // Получаем характеристики шаблона
    $stmt = $pdo->prepare("
    ...
    ");
    $stmt->execute([$templateId]);
    $existingAttributes = $stmt->fetchAll();
    ...
    logError('Template edit load error: ' . $e->getMessage(), ['template_id' => $templateId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($csrf_token)) {
        ...
        if (!empty($attributes)) {
            foreach ($attributes as $index => $attr) {
                ...
                if (!empty($attrErrors)) {
                    ...
                    $validatedAttributes[] = [
                        'id' => (int)($attr['id'] ?? 0), // ID для существующих характеристик
                        'name' => trim($attr['name']),
        ...
        // Проверяем, используется ли шаблон в товарах при изменении критичных полей
        if ($template['products_count'] > 0) {
            // Получаем существующие переменные
            $existingVariables = array_column($existingAttributes, 'variable');
            $newVariables = array_column($validatedAttributes, 'variable');
            
            // Проверяем удаление переменных
            $removedVariables = array_diff($existingVariables, $newVariables);
            if (!empty($removedVariables)) {
                $errors['attributes_general'] = 'Нельзя удалять характеристики из шаблона, по которому созданы товары (' . $template['products_count'] . ' шт.)';
            }
        }
        ...
        if (empty($errors)) {
            try {
                ...
                // Проверяем уникальность названия (исключая текущий шаблон)
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM product_templates WHERE name = ? AND id != ?");
                $stmt->execute([$formData['name'], $templateId]);
                if ($stmt->fetchColumn() > 0) {
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        SET name = ?, description = ?, formula = ?, status = ?, updated_at = CURRENT_TIMESTAMP
                        WHERE id = ?
                    ");
                    ...
                    if (!empty($validatedAttributes)) {
                        // Получаем ID существующих характеристик для обновления/удаления
                        $existingIds = array_column($existingAttributes, 'id');
                        $newIds = array_filter(array_column($validatedAttributes, 'id'));
                        
                        // Удаляем характеристики, которых нет в новом списке (только если нет товаров)
                        if ($template['products_count'] == 0) {
                            $idsToDelete = array_diff($existingIds, $newIds);
                            if (!empty($idsToDelete)) {
                                $placeholders = str_repeat('?,', count($idsToDelete) - 1) . '?';
                                $stmt = $pdo->prepare("DELETE FROM template_attributes WHERE id IN ($placeholders)");
                        ...
                        foreach ($validatedAttributes as $attr) {
                            if ($attr['id'] > 0) {
                                ...
                                $stmt = $pdo->prepare("
                                    ...
                                    SET name = ?, variable = ?, data_type = ?, options = ?, unit = ?, 
                                        is_required = ?, use_in_formula = ?, sort_order = ?
                                    WHERE id = ?
                                ");
                        ...
                        // Если характеристик нет, удаляем все (только если нет товаров)
                        if ($template['products_count'] == 0) {
                            $stmt = $pdo->prepare("DELETE FROM template_attributes WHERE template_id = ?");
                            $stmt->execute([$templateId]);
                        }
                    }
                    ...
                    $_SESSION['success_message'] = 'Шаблон "' . $formData['name'] . '" успешно обновлен';
                ...
                $pdo->rollBack();
                logError('Update template error: ' . $e->getMessage(), $formData);
                $errors['general'] = 'Произошла ошибка при обновлении шаблона';
            }
        }
    }
}
...
<div class="alert alert-warning">
    <i class="bi bi-exclamation-triangle"></i>
    <strong>Внимание!</strong> По этому шаблону создано <strong><?= $template['products_count'] ?></strong> товаров. 
    Удаление характеристик запрещено. Можно только добавлять новые или изменять существующие.
</div>
<?php endif; ?>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="template-info mb-3">
                        ...
                        <strong><?= formatDate($template['created_at'], 'd.m.Y H:i') ?></strong><br>
                        <small class="text-muted">
                            <?= e($template['first_name'] . ' ' . $template['last_name']) ?>
                        </small>
                    </div>
                    ...
                    <div class="template-info mb-3">
                        ...
                        <strong><?= formatDate($template['updated_at'], 'd.m.Y H:i') ?></strong>
                    </div>
                    <?php endif; ?>
                    
                    <div class="template-info mb-3">
                        <small class="text-muted">Товаров по шаблону:</small><br>
                        <strong class="<?= $template['products_count'] > 0 ? 'text-warning' : 'text-muted' ?>">
                            <?= $template['products_count'] ?> шт.
                        </strong>
                    </div>
                    
                    <div class="template-info">
                        <small class="text-muted">Характеристик:</small><br>
                        <strong class="text-primary" id="attributesCount"><?= count($existingAttributes) ?> шт.</strong>
                    </div>
                </div>
            </div>
...
<script type="application/json" id="existingAttributesData">
<?= json_encode($existingAttributes) ?>
</script>
...
<template id="attributeTemplate">
    <div class="attribute-item border rounded p-3 mb-3" data-index="">
        ...
        <div class="row">
            <!-- Скрытое поле ID для существующих характеристик -->
            <input type="hidden" class="attribute-id" name="attributes[][id]" value="">
...
function addAttribute(data = {}) {
    ...
    if (data.id) clone.querySelector('.attribute-id').value = data.id;
    ...
    if (data.options) {
        // Преобразуем JSON обратно в строку через запятую
        let optionsStr = data.options;
        if (typeof data.options === 'string' && data.options.startsWith('[')) {
            try {
                const optionsArray = JSON.parse(data.options);
                optionsStr = optionsArray.join(',');
            } catch (e) {
                optionsStr = data.options;
            }
        }
        clone.querySelector('.attribute-options').value = optionsStr;
    }
    if (data.unit) clone.querySelector('.attribute-unit').value = data.unit;
    if (data.is_required == 1) clone.querySelector('.attribute-required').checked = true;
    if (data.use_in_formula == 1) clone.querySelector('.attribute-formula').checked = true;
    ...
    // Автогенерация переменной из названия (только для новых характеристик)
    const nameInput = clone.querySelector('.attribute-name');
    const variableInput = clone.querySelector('.attribute-variable');
    
    if (!data.id) { // Только для новых характеристик
    ...
    clone.querySelector('.attribute-formula').addEventListener('change', function() {
        updateAvailableVariables();
    });
    ...
    const emptyMessage = document.getElementById('emptyAttributesMessage');
    if (emptyMessage) {
...
function removeAttribute(index) {
    ...
    if (item) {
        ...
        const emptyMessage = document.getElementById('emptyAttributesMessage');
        if (container.children.length === 1 && emptyMessage) { // только пустое сообщение
...
// Обновление счетчика характеристик
function updateAttributesCount() {
    const count = document.querySelectorAll('.attribute-item').length;
    const countElement = document.getElementById('attributesCount');
    if (countElement) {
...
document.addEventListener('DOMContentLoaded', function() {
    // Загружаем существующие характеристики
    const existingData = document.getElementById('existingAttributesData');
    if (existingData) {
        try {
            const attributes = JSON.parse(existingData.textContent);
